...
void loop() {
    ...
    if (motion_detected && motion_detection_toggle) {
        bot.sendMessage(chat_id, "Motion detected on Camera #1!", "");
...
void handleNewMessages(int new_messages_num){
  ...
  for (int i = 0; i < new_messages_num; i++){
    ...
    if (text == "/photo_cam1") {
        ...
        bot.sendMessage(chat_id, "Photo Taken on Camera #1", "");
    }

    if (text == "/toggle_motion_detection_cam1") {
        motion_detection_toggle = !motion_detection_toggle;
        if (motion_detection_toggle) {
            bot.sendMessage(chat_id, "Motion detection enabled on Camera #1", "");
        } else {
            bot.sendMessage(chat_id, "Motion detection disabled on Camera #1", "");
        }
    }
  }
}
